...
extern PRECISION vec2 agressivelyred;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(agressivelyred.r*0.1221), sin(agressivelyred.r*0.3512));

    vec3 agressivelyredVec = tex.rgb;
    if (agressivelyred.g > 0.0 || agressivelyred.g < 0.0) {
        agressivelyredVec.r = (tex.r * 0.393) + (tex.g * 0.769) + (tex.b * 0.189);
        agressivelyredVec.g = (tex.r * 0) + (tex.g * 0) + (tex.b * 0.);
        agressivelyredVec.b = (tex.r * 0) + (tex.g * 0) + (tex.b * 0);
    }

    tex = vec4(agressivelyredVec, tex.a);
